...
temp_z=read.csv("daily_temp.csv")$national_tmax
...
for (k in (n_total-14):n_total){
  ...
  if (!any(is.na(temp_z))){
    z_cut <- temp_z[1:k]
    z_temp_1 <- as.numeric(z_cut >= 31.7)[-c(1,nn)]
    z_temp_2 <- as.numeric(z_cut <= 14.6)[-c(1,nn)]
  ...
  if (basis_name == "Legen"){
    ...
    x_score = cbind(x_scale1,x_scale2)
  ...
  lamb=gcv_lamb(prec_cut,x,c+temp_dim,R_med)$lamb
  R=lamb*R_med
  theta_hat=as.vector(solve(t(x)%*%x/n+R)%*%t(x)%*%prec_cut/n)
  residual <- as.vector(prec_cut - x%*%theta_hat)
  res_quan <- sort(residual)[c(floor(0.025*n),floor(0.975*n))]
...
mean(mse_func) # 0.00845577 decrease a little when adding temp 0.0080
library(ggplot2)
...
ggplot(data = data, aes(x = Day)) + 
  ...
  labs(x="Day",y="log(Demand)")+ylim(-0.25,0.22) + 
  ...
  scale_x_continuous(breaks=1:15, labels=rep(c("M","T","W","T","F"), length.out=15)) +
