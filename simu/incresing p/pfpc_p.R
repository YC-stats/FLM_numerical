...
#n=800 p=3 (l=0.15,u=1.15) p=2 (l=0.25,u=0.85) p=1 (l=0.15,u=0.75)
#n=400 p=3 (l=0.5,u=1.15)
l=0.15 
...
for (k in 1:repli){
  ...
  x_fpc=array(0,c(res,rank1,p))
  for (i in 1:p){
    cvp=0
    mm=0
    xx=gMA0(n,d2,a,decay=(1:a)^(-1.2)) #n*a
    x_mat=xx%*%basis #n*res
    x_med=prcomp(x_mat,rank=rank1,center=TRUE)
    x_deri_2=xx%*%sweep(basis,1,c(0,-sqrt(2)*(pi*(1:(a-1)))^2),"*")
    x_fpc[,,i]=as.matrix((x_med$rotation)*sqrt(res)) #res*rank1
    ...
    c[i]=2*mm
    f_hat=as.vector(apply(x_score[,1:c[i]],2,sd))
    for (j in 1:res)
      x_deri_fpc[,j]=ginv(x_score)%*%x_deri_2[,j]
    PP=t(x_deri_fpc[,1:c[i]])%*%x_deri_fpc[,1:c[i]]/res
    pen_mat=list(sweep(sweep(PP,1,f_hat,"/"),2,f_hat,"/"))
    P_comb=append(P_comb,pen_mat)
    x_scale=sweep(x_score[,1:c[i]],2,f_hat,"/")
    xx_comb=cbind(xx_comb,xx)
    x_comb=cbind(x_comb,x_scale)# scaled version
  ...
  if (p==1){
    beta_hat[,1]=as.vector(x_fpc[,1:c[1],1]%*%b_hat)
  }else if (p==2){
    beta_hat[,1]=as.vector(x_fpc[,1:c[1],1]%*%b_hat[1:c[1]])
    beta_hat[,2]=as.vector(x_fpc[,1:c[2],2]%*%b_hat[(c[1]+1):c_length])
  }else{
    beta_hat[,1]=as.vector(x_fpc[,1:c[1],1]%*%b_hat[1:c[1]])
    beta_hat[,2]=as.vector(x_fpc[,1:c[2],2]%*%b_hat[(c[1]+1):(c[1]+c[2])])
    beta_hat[,3]=as.vector(x_fpc[,1:c[3],3]%*%b_hat[(c[1]+c[2]+1):c_length])
  }
  ...
  ####construct SCB#####
  med=pboots_fpc(S,x,f,t,m,c,time,P,x_fpc,g,p)
  count1=0
  count2=0
  if (g==1){
    r=sort(med)
...
width1=width_1[,1]
width2=width_2[,1]
print(round(mean(width1),4))
print(round(mean(width2),4))
