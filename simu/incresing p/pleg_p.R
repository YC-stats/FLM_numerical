...
xx=array(0,c(n,a,p))
lamb=rep(0,repli)
basis=matrix(0,nrow=a,ncol=res)
tilde_t=2*t-1
for (i in 1:a)
  basis[i,]=sqrt(2*i-1)*leg_constru(tilde_t,i-1)
g=0
beta_true=cbind()
b_comb=cbind()
...
#n=800 p=3 (l=0.25,u=1) p=2 (l=0.25,u=0.85) p=1 (l=0.25,u=1)
#n=400 p=3 (l==0.5,u=1) p=2 (l=0.5,u=1.5) p=1 (l=0.15,u=1)
l=0.15
u=1
for (k in 1:repli){
  f=cbind()
  ...
  for (i in 1:p){
    ...
    if (c[i]==6){
      PP=diag(c(0,0,720,8400,49680,203280)/(f_hat^2),c[i],c[i])
      PP[3,5]=1440*sqrt(5)/f_hat[3]/f_hat[5]
      PP[6,4]=3360*sqrt(77)/f_hat[4]/f_hat[6]
      PP[4,6]=3360*sqrt(77)/f_hat[4]/f_hat[6]
      PP[5,3]=1440*sqrt(5)/f_hat[3]/f_hat[5]
    }
    if (c[i]==4){
      PP=diag(c(0,0,720,8400)/(f_hat^2),c[i],c[i])
    }
    pen_mat=list(PP)
