##########multipier bootstrap#############
pboots_fpc<-function(S,x,f,t,m,c,time,R,basis_fun,gg,p){
  basis=array(0,c(6,res,p))
  for (i in 1:p)
    basis[,,i]=t(basis_fun[,,i])
  ...
  if (p==1){
    A_hat=sweep(basis[1:c[1],,1],1,f[1:c[1]],"/")
    Q=t(A_hat)%*%Sigma_inv%*%uboots
  }else if (p==2){
    A_hat=matrix(0,c_length,p*res)
    A_hat[1:c[1],1:res]=sweep(basis[1:c[1],,1],1,f[1:c[1]],"/")
    A_hat[(c[1]+1):c_length,(res+1):(2*res)]=
      sweep(basis[1:c[2],,2],1,f[(c[1]+1):(c[1]+c[2])],"/")
    Q=t(A_hat)%*%Sigma_inv%*%uboots
  }else{
    A_hat=matrix(0,c_length,p*res)
    A_hat[1:c[1],1:res]=sweep(basis[1:c[1],,1],1,f[1:c[1]],"/")
    ...
    A_hat[(c[1]+c[2]+1):c_length,(2*res+1):(3*res)]=
      sweep(basis[1:c[3],,3],1,f[(c[1]+c[2]+1):(c[1]+c[2]+c[3])],"/")
    Q=t(A_hat)%*%Sigma_inv%*%uboots
  }
  if (gg==1){
